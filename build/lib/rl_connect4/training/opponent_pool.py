...
def _masked_predict(
    model: PredictModel, obs: np.ndarray, action_mask: np.ndarray
) -> int:
...
def make_model_policy(model: PredictModel) -> OpponentPolicy:
    def _policy(obs: np.ndarray, action_mask: np.ndarray) -> int:
        return _masked_predict(model, obs, action_mask)

    return _policy


def make_checkpoint_policy(checkpoint_path: str | Path) -> OpponentPolicy:
...
class OpponentPool:
    ...
    def set_current_model(self, model: PredictModel) -> None:
        self._current_policy = make_model_policy(model)

    def refresh_historical(self, checkpoint_paths: list[Path]) -> None:
        self._historical_policies = [
            make_checkpoint_policy(path) for path in checkpoint_paths
        ]
    ...
    def set_elite_checkpoint(self, checkpoint_path: Path | None) -> None:
        self._elite_policy = (
            ...
            else make_checkpoint_policy(checkpoint_path)
        )
